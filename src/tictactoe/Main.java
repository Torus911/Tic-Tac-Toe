...
public class Main {
    ...
    public static void main(String[] args) {
        while (true) {
            System.out.println("\nWrite start and choose players and levels for ai players (easy,medium,hard) e.g. start user medium");
            System.out.println("Write exit to finish");
            System.out.print("Input command: ");
            String command = scan.nextLine();
            System.out.println("Coordinates scheme:\n" +
                    "---------------\n" +
                    "|1 1  1 2  1 3|\n" +
                    "|2 1  2 2  2 3|\n" +
                    "|3 1  3 2  3 3|\n" +
    ...
    private static int findGameEndingMove(char c) {
        ...
        for (int i = 0; i < grid.length; i += 3) {
            ...
            if (line.chars().filter(l -> l == c).count() == 2 && line.contains(" ")) {
                ind = i + line.indexOf(' ');
                break;
            }
        ...
        for (int i = 0; i < 3; i++) {
            ...
            if (column.chars().filter(l -> l == c).count() == 2 && column.contains(" ")) {
                ind = i + 3 * column.indexOf(' ');
                break;
            }
        }
        ...
        if (dia1.chars().filter(l -> l == c).count() == 2 && dia1.contains(" ")) {
            ind = dia1.indexOf(' ') * 4;
        }
        if (dia2.chars().filter(l -> l == c).count() == 2 && dia2.contains(" ")) {
            ind = dia2.indexOf(' ') * 2 + 2;
        }
        return ind;
    }
